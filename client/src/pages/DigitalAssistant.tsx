...
export default function DigitalAssistant() {
  ...
  const { startListening, stopListening, isListening, transcript, error: speechError, setTranscript, lang: sttLang, setLang: setSttLang } = useSpeechToText("ar");

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  // Update STT language when current language changes
  useEffect(() => {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(currentLang === "ar" ? "فشل في الاتصال بالمساعد" : "Failed to connect to assistant");
      }

      const data = await response.json();
      const assistantContent = data.content || (currentLang === "ar" ? "عذراً، لم أتمكن من معالجة طلبك. حاول مرة أخرى." : "Sorry, I couldn't process your request. Try again.");
      ...
      // Auto-speak the response with explicit language
      if (autoSpeak) {
        setTimeout(() => speak(assistantContent, 0.9, currentLang), 300);
      }
    } catch (err: any) {
      console.error("Assistant error:", err);
      const errorContent = currentLang === "ar" ? "عذراً، حدث خطأ في الاتصال. يرجى المحاولة مرة أخرى." : "Sorry, connection error. Please try again.";
      ...
      toast.error(currentLang === "ar" ? "فشل في الاتصال بالمساعد" : "Failed to connect");
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="rounded-2xl border border-border/50 bg-card shadow-lg overflow-hidden">
            ...
            <div className="p-4 md:p-5 border-b border-border/50 bg-amber-50/50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  title={currentLang === "ar" ? "اضغط للتبديل إلى الإنجليزية" : "Click to switch to Arabic"}
                >
                ...
                <Button
                  ...
                  aria-label={autoSpeak ? (currentLang === "ar" ? "إيقاف القراءة التلقائية" : "Disable auto-speak") : (currentLang === "ar" ? "تفعيل القراءة التلقائية" : "Enable auto-speak")}
                >
            ...
            <div className="p-3 border-b border-border/30 flex gap-2 overflow-x-auto">
              {quickActions.map((action) => {
                // Translate quick actions based on current language
                let label = action.label;
                let actionText = action.action;
                if (currentLang === "en") {
                  const translations: Record<string, { label: string; action: string }> = {
                    "اقرأ لي السؤال": { label: "Read Question", action: "Read the current question clearly" },
                    "ساعدني في الإجابة": { label: "Help Me Answer", action: "Help me understand the current question and give me hints" },
                    "راجع إجاباتي": { label: "Review Answers", action: "Review all my answers and tell me if I missed anything" },
                    "كيف أستخدم المنصة؟": { label: "How to Use", action: "Explain how to use Basira platform step by step" },
                  };
                  const trans = translations[action.label];
                  if (trans) {
                    label = trans.label;
                    actionText = trans.action;
                  }
                }
                return (
                  <button
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder={isListening ? (currentLang === "ar" ? "جاري الاستماع..." : "Listening...") : (currentLang === "ar" ? "اكتب رسالتك هنا أو استخدم الميكروفون..." : "Type your message or use the microphone...")}
                    disabled={isLoading}
                    ...
                    aria-label={currentLang === "ar" ? "حقل إدخال الرسالة" : "Message input field"}
                  />
                </div>
                <Button
                  ...
                  aria-label={currentLang === "ar" ? "إرسال الرسالة" : "Send message"}
                >
